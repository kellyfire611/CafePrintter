...
// ReSharper restore UnusedMember.Global
	{
		...
		private void PopulateReportInfo(ExceptionReportInfo reportInfo)
		{
			...
			reportInfo.CustomTitle = "Hệ thống phát sinh sự cố. Vui lòng liên hệ quản trị hệ thống để được hỗ trợ.";

			txtExceptionMessageLarge.Text = 
				txtExceptionMessage.Text =
				!string.IsNullOrEmpty(reportInfo.CustomTitle) ? reportInfo.CustomTitle : reportInfo.Exceptions[0].Message;
			...
			lblContactCompany.Text = string.Format("Nếu có vấn đề xảy ra, vui lòng liên hệ {0} để được hỗ trợ.", reportInfo.CompanyName);
		...
		{
			get { return _autoSendEmail; }
			set { _autoSendEmail = value; }
		}
		...
		private void ExceptionReportView_Shown(object sender, EventArgs e)
		{
			if (this.AutoSendEmail)
			{
				btnSimpleEmail.PerformClick();
			}
		}
